...
The first hardening increment enforces uniqueness for startup, login, and
handshake candidates: multiple matching literals in one method are corroborating
signals, while matching different methods is an explicit ambiguity failure
instead of silent last-match-wins behavior. The historical login-acknowledgement
false positive is also pinned as a regression test.
...
1. prove the SourceScanner ambiguity invariant against unit tests and the real
   current-stable patch + boot probe;
