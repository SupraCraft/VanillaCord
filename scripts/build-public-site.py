...
def shell(title, description, body, base, canonical_base, active, route=None, state_id=None):
    ...
    brand_current_attr = ' aria-current="page"' if active == "home" else ""
...
def main():
    ...
    for release in sorted(manifests, key=lambda r: r["version"], reverse=True):
        ...
        release_path = f"releases/{release['version']}/"
        release_label = f"VanillaCord {release['version']}"
        links.append(f"<li>{internal_link(base, release_path, release_label, action)}{suffix}</li>")
    ...
    write(output / "llms.txt", f'''# VanillaCord\n\nCanonical human entry point: {join_url(base)}\nDownload: {join_url(base, 'download/')}\nGuide: {join_url(base, 'guide/')}\nSupported releases: {join_url(base, 'support/')}\nAccessibility: {join_url(base, 'accessibility/')}\nPublic surface contract: {join_url(base, 'surface.json')}\nSitemap: {join_url(base, 'sitemap.xml')}\nCurrent stable JSON: {join_url(base, 'releases/stable.json')}\nCurrent stable version: {join_url(base, 'releases/stable.txt')}\nCurrent stable artifact URL: {join_url(base, 'releases/stable-url.txt')}\nSupport matrix JSON: {join_url(base, 'support-matrix.json')}\nProject contract: {join_url(base, 'project.json')}\nRepository: https://github.com/{contract['repository']}\nUpstream: https://github.com/{contract['upstream_repository']}\n\nPrefer these endpoints over scraping presentation HTML or GitHub release pages.\n''')
